...
def safe_label(value, default="") -> str:
    ...
    # 处理已存入数据库的 dict 字符串: "{'value': 'WT', 'label': 'Web Terminal'}"
    s = str(value)
...
def should_suppress(alert_type: str, ip: str, cfg: dict, username: str = "") -> bool:
    """判断是否应抑制告警，返回 True 表示抑制（不发）"""
    key = f"{alert_type}:{ip}:{username}"
    suppress_sec = cfg.get("alert_suppress_seconds", 1800)
    now = datetime.now()

    last = _alert_cooldown.get(key)
    if last and (now - last).total_seconds() < suppress_sec:
        return True
...
def collect_commands_by_session(client: JumpServerV2Client, conn: sqlite3.Connection, cfg: dict, session_id: str):
    ...
    for item in client.paginate("terminal/commands/", {"session_id": session_id}):
        ...
        if risk["level"] >= 2:
            ...
            if not should_suppress("dangerous_cmd", cmd_ip, cfg, safe_str(item.get("user"))):
                msg = (
                    f"**用户**: {item.get('user')}\n"
                    f"**资产**: {item.get('asset')}\n"
                    f"**IP**: {cmd_ip}\n"
                ...
                logger.info(f"高危命令告警已抑制(30min内): {cmd_ip}")
...
def cleanup_old_data(conn: sqlite3.Connection, cfg: dict):
    ...
    conn.commit()
    conn.execute("DELETE FROM sync_state WHERE key LIKE 'alert_cooldown:%' AND value < ?", (cutoff,))
    conn.commit()
...
def collect_sessions(client: JumpServerV2Client, conn: sqlite3.Connection, cfg: dict):
    ...
    if count:
        logger.info(f"会话记录: 采集 {count} 条 (新增敏感会话: {sensitive_new})")
...
def collect_sensitive_fast(client: JumpServerV2Client, conn: sqlite3.Connection, cfg: dict):
    ...
    for session_id, username, asset, asset_ip, protocol, date_start, \
        ...
        if not should_suppress("session_report", asset_ip, cfg, username):
            msg = (
                f"**用户**: {username}\n"
                f"**资产**: {asset}\n"
                f"**IP**: {asset_ip}\n"
            ...
            logger.info(f"[敏感资产] 会话结束报告已抑制(30min内): {asset_ip}")
...
def main():
    ...
    logger.info(f"告警抑制: 同IP同类型 {cfg['alert_suppress_seconds'] // 60} 分钟")
